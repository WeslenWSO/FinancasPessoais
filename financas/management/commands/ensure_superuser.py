...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        email = os.environ.get('DJANGO_SUPERUSER_EMAIL', 'admin@financaspessoal.local').strip()

        if not username or not password:
            self.stdout.write('DJANGO_SUPERUSER_USERNAME/PASSWORD não definidos — ignorando.')
            return
